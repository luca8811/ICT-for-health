...
#               warn_bad_lines=True)
xx=pd.read_csv("/Users/lucabernardi/Desktop/ICT for smart societies/ICT for health/LAB5/Chronic_Kidney_Disease/chronic_kidney_disease_v2.arff",sep=',',
    skiprows=29,names=feat_names, 
    header=None,na_values=['?','\t?'],)
Np,Nf=xx.shape
...
#%%
miss_values = xx.isnull().sum(axis=1) #how many missing values are present in each row.
...
for kk in range(Np):
    ...
    X_norm.loc[kk, mask] = ytest_norm # substitute nan with regressed values
...
# Use the trained classifier to predict on the new dataset x_new
test_pred_new = clfXtrain.predict(x_new.drop('classk', axis=1))

# Calculate accuracy and confusion matrix for the new dataset
accuracy_new = accuracy_score(x_new.classk, test_pred_new)
conf_matrix_new = confusion_matrix(x_new.classk, test_pred_new)

# Print the accuracy and confusion matrix for the new dataset
print('Performance of the decision tree on the new dataset:')
print('accuracy =', accuracy_new)
print(conf_matrix_new)

# Plot the decision tree for the new dataset
plt.figure(figsize=(10, 15))
tree.plot_tree(clfXtrain, feature_names=feat_names[:24],
               class_names=target_names, rounded=True,
               proportion=False, filled=True)
plt.savefig('fig_new_dataset.png')
